...
powersim <- function(es, n = 49, a = 0.05, nsim = 1E4) {
...
res <- foreach(ii = es, .combine = c) %dopar% powersim(ii)  ## power
stopCluster(cl)
...
rois.tost.targt.vs.distr <- stats.tosts[roi.hemi %in% rois.pref.targt.vs.distr & param == "distractor", ] %>%
  ungroup %>%
  mutate(p.fdr = p.adjust(p, method = "fdr"), p.holm = p.adjust(p, method = "holm")) %>%
  filter(p.fdr < 0.05) %>%
  .$roi.hemi

stats.tosts[param == "distractor" & roi.hemi %in% rois.tost.targt.vs.distr]
...
rois.tost.targt.vs.incon <- stats.tosts[roi.hemi %in% rois.pref.targt.vs.incon & param == "incongruency", ] %>%
  ungroup %>%
  mutate(p.fdr = p.adjust(p, method = "fdr"), p.holm = p.adjust(p, method = "holm")) %>%
  filter(p.fdr < 0.05) %>%
  .$roi.hemi
...
stats.tosts[param == "incongruency" & roi.hemi %in% rois.tost.targt.vs.incon]
...
rois.tost.distr.vs.incon <- stats.tosts[roi.hemi %in% rois.pref.distr.vs.incon & param == "incongruency", ] %>%
  ungroup %>%
  mutate(p.fdr = p.adjust(p, method = "fdr"), p.holm = p.adjust(p, method = "holm")) %>%
  filter(p.fdr < 0.05) %>%
  .$roi.hemi
...
stats.tosts[param == "incongruency" & roi.hemi %in% rois.tost.distr.vs.incon]
...
rois.tost.incon.vs.distr <- stats.tosts[roi.hemi %in% rois.pref.incon.vs.distr & param == "distractor", ] %>%
  ungroup %>%
  mutate(p.fdr = p.adjust(p, method = "fdr"), p.holm = p.adjust(p, method = "holm")) %>%
  filter(p.fdr < 0.05) %>%
  .$roi.hemi

stats.tosts[param == "distractor" & roi.hemi %in% rois.tost.incon.vs.distr]
...
rois.tost.incon.vs.targt <- stats.tosts[roi.hemi %in% rois.pref.incon.vs.targt & param == "target", ] %>%
  ungroup %>%
  mutate(p.fdr = p.adjust(p, method = "fdr"), p.holm = p.adjust(p, method = "holm")) %>%
  filter(p.fdr < 0.05) %>%
  .$roi.hemi
...
stats.tosts[param == "target" & roi.hemi %in% rois.tost.incon.vs.targt]
