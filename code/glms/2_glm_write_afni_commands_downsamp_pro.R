...
fname.bias.incon <- paste0(bias.items.incon, "_acc-only_downsamp")

dmat.events <- cbind(
  .time.opt = "stim_times",
  .label    = c(bias.items.incon, bias.items.con, "pc50_i", "pc50_c", "nuisance", "congr_modelout", "incon_modelout"),
  .filename = c(
    fname.bias.incon, fname.bias.con, 
    paste0(
      c("pc50_i", "pc50_c", "nuisance", "congr_modelout", "incon_modelout"),
      "_acc-only"
    )
  ),
  .fun      = "'BLOCK(1,1)'"
...
stroop.pro.subj.nums <- stroop %>% filter(session == "pro") %>% split(list(.$subj)) %>% purrr::map_dbl(nrow)
stroop.pro.subjs <- names(stroop.pro.subj.nums)[stroop.pro.subj.nums == 216]
...
lapply(
  ...
  function(x)
    write.afni.cmds(
      ...
      .dmat         = dmat,
      code.dir      = "code/glms/",
      out.folder    = "pro_bias_acc-only_downsamp",
      .dir.image    = dir.nil.dmcc2.afni,
