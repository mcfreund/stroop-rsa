...
write.afni.cmds.runwise <- function(
  subj, session, run, .dmat, code.dir, out.folder, .dir.image = dir.nil.dmcc2.afni, .dir.analysis
) {
  # subj <- "132017"; session <- "proactive";
  # .dir.image = dir.nil.dmcc2.afni
  # .dir.analysis = file.path(dir.freund.external,"stroop-rsa", "afni-analysis_group-201902")
  ...
  if (run == 1) run.long <- "1_AP" else run.long <- "2_PA"
  ...
  writeLines(
    c(
      ...
      paste0("  -input ", image.filename, " \\"),
      "  -polort A -float \\",
      paste0("  -censor ", dir.input, "/movregs_FD_mask", "_run", run, ".txt \\"),
      paste0("  -num_stimts ", nrow(dmat), " \\"),  ## plus two for "block" regressors
      
      ## begin design mat
      paste0(
        "  -", dmat[, ".time.opt"]," ", 1:nrow(dmat), " ", 
          path.initial, dmat[, ".filename"], "_run", run, ".txt ", dmat[, ".fun"], 
        "  -stim_label ", 1:nrow(dmat), " ", dmat[, ".label"], " \\"
      ),
      paste0("  -ortvec ", dir.input, "/motion_demean_", session, "_run", run, ".1D movregs \\"),
      "  -x1D X.xmat.1D -xjpeg X.jpg -nobucket",
      "\n",
...
dmat.events <- cbind(
  ...
  .filename = paste0(c(bias.items, "pc50_i", "pc50_c", "nuisance"), "_acc-only"),
  .fun      = "'BLOCK(1,1)'"
)
dmat.blocks <-   rbind(
