---
title: "validation set analyses: group-level"
...
stats.subjs.super <- read_subj_stats(subjs = c(subjs.analysis, subjs.validation))
stats.subjs.mmp <- read_subj_stats(roi.set = "mmp", subjs = c(subjs.analysis, subjs.validation))
rsarray.line <- read_simil_mats(subjs = setdiff(c(subjs.analysis, subjs.validation), "562345"), tfrm = "residual-line")  ## for MDS
rsarray.rank <- read_simil_mats(subjs = setdiff(c(subjs.analysis, subjs.validation), "562345"), tfrm = "residual-rank")  ## for noise ceiling
...
stats.subjs.super %>%
  ...
  stat_summary(fun.data = mean_cl_boot, size = 2, position = position_dodge(width = 0.5), fun.args = list(B = 1E4)) +
  
  facet_grid(vars(subj.set)) +
...
stats.group.super <- stats.subjs.super %>%
  
  filter(param %in% params.interest, roi %in% c("dlpfc", "dmfc", "lppc")) %>%
  
  group_by(param, roi.hemi, subj.set) %>%
  summarize(
    b = mean(beta),
    p = wilcox.test(beta, alternative = "greater")$p.value
  )

stats.group.super %>% filter(subj.set == "train", p < 0.05)
stats.group.super %>% filter(subj.set == "test1", p < 0.05)
stats.group.super %>% filter(subj.set == "test2", p < 0.1)
stats.group.super %>% filter(subj.set == "test2") %>% View
...
fit.super <- lmer(
  ...
  stats.subjs.super %>% filter(roi %in% c("dlpfc", "dmfc", "lppc"), subj.set == "test2")
)
summary(fit.super)
...
table.group <- data.frame(
  ...
  p  = glht.super$test$pvalues  ## for one-sided hypothesis test
)
rownames(table.group) <- NULL

table.group.means <- table.group[1:18, ]
table.group.means$p <- ifelse(table.group.means$b > 0, table.group.means$p / 2, 1 - table.group.means$p/2)  ## one-sided test
