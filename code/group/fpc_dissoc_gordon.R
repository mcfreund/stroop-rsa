...
if (interactive()) {
  ...
  source(here::here("code", "group", "fpc_dissoc.R"))
  stats.subjs.gordon <- read_subj_stats(roi.set = "gordon")
  
}

#' ### plot means from all ROIs

#+ fpc-dissoc-gordon_plot-all
unique(stats.subjs.gordon$roi)
rois.gordon <- c("dorsalattn_L", "dorsalattn_R", "cingulooperc_L", "cingulooperc_R", "frontoparietal_L", "frontoparietal_R")

stats.subjs.gordon %>%
  
  filter(param %in% params.interest, roi %in% rois.gordon) %>%
