...
stats.subjs.super <- 
  fread(
    ...
    filter(is.analysis.group, y == "rank", param %in% c("target", "distractor", "incongruency")) %>%
...
contrasts.prelim <- rbind(
  ...
  "smmouth-evis|target"     = c(0, 0, -1, 1),
  ## within-parcel contrasts:
  "distractor-target|evis" = c(1, 0, -1, 0),
  "target-distractor|smmouth"= c(0, -1, 0, 1),
  ## interaction:
...
p.means.prelim <- means.prelim %>%
  ...
  annotate(
    ...
    hjust = 0, vjust = 1, size = label.size*2, fontface = "bold"
    ) +
  annotate(
    geom = "text", x = 0.5, y = 0.05, label = "V1", color = "#b2182b",
    hjust = 0, vjust = 1, size = label.size*2, fontface = "bold"
    ) +
  
  annotate(
    geom = "text", x = 2, y = 0.05,
    label = paste0("p(distr.-targt.)>0 = ", round(glht.prelim$test$pvalues["distractor-target|evis"], 4)),
    color = "#b2182b",
    hjust = 0.5, vjust = 1, size = label.size, fontface = "bold"
    ) +

  annotate(
    geom = "text", x = 1.25, y = 0.1,
...
# ggsave(
#   here("docs", "papers", "ms_v1", "figs", "group", "prelim.pdf"), 
#   p.means.prelim, device = "pdf", width = figwidth, height = 3
#   )
...
table.group.alt <- data.frame(
  contrast = names(glht.super.all.alt$test$coefficients),
