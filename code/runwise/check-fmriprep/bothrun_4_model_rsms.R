...
for (set.i in sets.of.rois) {
  ...
  rsarray <- readRDS(
    here(
      "out", "rsa", "obsv",
      # paste0("rsarray_pro_bias_acc-only_", set.i, ".rds")
      paste0("rsarray_pro_bias_acc-only_downsamp_", set.i, ".rds")
    )
  )
  ...
  fwrite(
    stats.subjs,
    here("out", "rsa", "stats",  paste0("subjs_pro_bias_acc-only_downsamp_", set.i, ".csv"))
    )
