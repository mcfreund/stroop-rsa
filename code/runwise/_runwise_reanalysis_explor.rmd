...
if (type == "orig") {
  fname <- "subjs_pro_bias_acc-only_fmriprep_mmp_residual.csv"
} else if (type == "crun") {
  fname <- "subjs_pro_bias_acc-only_fmriprep_im_run_downsampled_crun_mmp.csv"
} else if (type == "both") {
  fname <- "subjs_pro_bias_acc-only_fmriprep_im_run_downsampled_both_mmp.csv"
} else if (type == "wnrun") {
  fname <- "subjs_pro_bias_acc-only_fmriprep_im_run_downsampled_wnrun_mmp.csv"
} else if (type == "cves") {
  fname <- "subjs_pro_bias_acc-only_fmriprep_runwise_cv-euclidean-stand_mmp.csv"
}
...
(rois.distr <- stats.group.mmp[param == "distractor" & p.fdr < 0.05, roi])
...
# stats.group.mmp %>% filter(param == "incongruency") %>% View
...
fname.overlay.coding <- c(
  here("out", "runwise", "target_", type, ".png"), 
  here("out", "runwise", "distractor_", type, ".png"),
  here("out", "runwise", "incongruency_", type, ".png")
)
...
if (all(file.exists(fname.overlay.coding))) {
  ...
  cifti.convert(
    ...
    fname.overlay  = paste0("group_target_", type, "_pval"),
  ...
  cifti.convert(
    path.wb = "C:/Program Files/workbench/bin_windows64",
    fname.overlay  = paste0("group_distractor_", type, "_pval"),
    dir.to.write   = here("out", "runwise"),
  ...
  cifti.convert(
    path.wb = "C:/Program Files/workbench/bin_windows64",
    fname.overlay  = paste0("group_incongruency_", type, "_pval"),
    dir.to.write   = here("out", "runwise"),
...
table.group.mmp.targt
table.group.mmp.incon
table.group.mmp.distr
