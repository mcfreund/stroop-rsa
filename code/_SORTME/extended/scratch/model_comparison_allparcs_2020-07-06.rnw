...
stats.subjs.tdic %<>% mutate(id = paste(roi, "_", param))
d <- stats.subjs.tdic
...
elnet05.cv <- cv.glmnet(w[, -grep("stroop", colnames(w))], w[, "stroop"])
elnet05 <- glmnet(w[, -grep("stroop", colnames(w))], w[, "stroop"], alpha = 1, lambda = elnet05.cv$lambda.min)
plot(elnet05.cv)
coef(elnet05)
plot(elnet05)

plot(elnet05.cv)
coef(elnet05.cv, lambda = elnet05.cv$lambda.1se)


# covtest for selective inference ----

library(selectiveInference)
all(svd(w[, -grep("stroop", colnames(w))])$d > 0)  ## check assumption
path <- lar(w[, -grep("stroop", colnames(w))], w[, "stroop"])
pathinf <- larInf(path)
pathinf
...
# library("lars")
stab.lasso <- stabsel(
  x = w[, -grep("stroop", colnames(w))], y = w[, "stroop"], fitfun = glmnet.lasso, 
  cutoff = 0.75, PFER = 1
  )
stab.lasso
plot(stab.lasso)
