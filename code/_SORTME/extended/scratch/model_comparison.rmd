...
stats.subjs.tdic <- fread(
  here("out", "rsa", "stats", paste0("subjs_pro_bias_acc-only_masks_pearson_residual_glm-tdclust.csv"))
)
...
## ips_R ??
## smmouth: averaged across hemisphere
...
d <- stats.subjs.tdic %>% 
  filter(param %in% c("distractor", "conclust", "target")) %>%
  filter(
    roi.set == "anatfunc" | superparcel %in% c("vwfa", "smmouth")
  )
