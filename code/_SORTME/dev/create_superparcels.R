## about ----
## 
## create 'superparcels' for brain--behavior analysis
...
md <- list(
  ...
  extended   = c(
    "TE1m", "TE1p", "PGs", "PFm", "AIP", "MIP", "LIPd", "IP1", "IP2", "s6-8", "i6-8", "a9-46v", "FOP5",
    "AVI", "11l", "a10p", "p10p", "a47r", "p47r"
    )
)
...
setdiff(unlist(anatomical), rois)

setdiff(rois, unlist(anatomical))

setdiff(unlist(anatomical), unlist(original))

setdiff(unlist(anatomical), unlist(md))

setdiff(unlist(md), unlist(anatomical))

setdiff(unlist(md), rois)
