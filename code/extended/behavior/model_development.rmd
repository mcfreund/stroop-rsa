...
stroop.pro.rt$resid.p <- resid(fit1.het.reml, type = "p")

plot(stroop.pro.rt$resid.p)
...
plot(
  stroop.pro.rt$resid.p,
  col = ifelse(stroop.pro.rt$is.far.out, "firebrick", "black")
  )

sum(stroop.pro.rt$is.far.out)

## outlier exclusions per subject * condition

counts.subj.out <- table(stroop.pro.rt$subj, stroop.pro.rt$trial.type, stroop.pro.rt$is.far.out) %>% as.data.frame
tidyr::spread(counts.subj.out, Var2, Freq) %>% 
  filter(Var3 == TRUE) %>% dplyr::select(-Var3) %>%
  rename(subj = Var1) %>%
  kable(digits = 2)

## refit model

fit1.het.reml.trim <- update(fit1.het.reml, subset = !is.far.out)

summary(fit1.het.reml.trim)

plot(
  coef(fit1.het.reml)[, "trial.typeincon"],
  coef(fit1.het.reml.trim)[, "trial.typeincon"]
)

qqnorm(coef(fit1.het.reml)[, "trial.typeincon"])

qqnorm(coef(fit1.het.reml.trim)[, "trial.typeincon"])

```

## re-estimate internal consistency

```{r split-half-reliability_model_trimmed}
...
```{r split-half-reliability_posterior-modes_trimmed}
